...
public partial class BattleCharacter : CharacterBody2D
{
	...
	[Export] public SelectActions selectActions;
	[Export] public Godot.Vector2 DodgeDir, BattleOffset;
			...
			if (Input.IsActionJustPressed("SelectedKey" + aux.PartyId))
			{
				switch (battleState)
				{
					...
					case BattleState.Dodging:
						if (actionState == ActionState.isIdle && DodgeDir != Godot.Vector2.Zero && (DodgeDir.X * DodgeDir.Y == 0))
						{
	...
	public void AddStatMultiplier(float Multiplier, int Timer, int Stat)
	{
		...
		Character.ShowTextLabel($"{StatChanged} {UpDown}", Character.Base.TextEffectColor);
	}
